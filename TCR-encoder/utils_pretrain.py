...
class TestbedDataset(Dataset):
    def __init__(self, root='tmp', dataset='_drug1'):
        ...
        if os.path.isfile('data/pretrain/data/' + dataset + '.pt'):
            print('Pre-processed data found: {}, loading ...'.format('data/pretrain/data/' + dataset + '.pt'))
            self.data = torch.load('data/pretrain/data/' + dataset + '.pt')
            print("1")
        else:
            print('Pre-processed data {} not found, doing pre-processing...'.format('data/pretrain/data/' + dataset + '.pt'))
    ...
    def process(self, root, dataset):
        ...
        compound_iso_smiles += list(df['CDR3'])
        ...
        for smile in compound_iso_smiles:
            ...
            atoms = str2list(smile)
            # make the graph ready for PyTorch Geometrics GCN algorithms:
            org_dict, smiles = get_dict(smile, atoms, org_dict)


            data_list.append(smiles)
            print('tcrs ', count, smiles)


        print('Saving to file.')
        with open("data/pretrain/data/" + dataset + "_dict.txt", 'w') as f:
            for k, v in org_dict.items():
                f.write(str(k) + ' ' + str(v) + '\n')
            f.close()
        # save preprocessed data:
        # save preprocessed data:
        torch.save(data_list, 'data/pretrain/data/' + dataset + '.pt')
